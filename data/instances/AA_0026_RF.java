...
public class AA_0026_RF {
  ...
  private static void assertFormattingAlwaysEmitsUsLocale(Locale locale) {
    ...
    try {
      // The patterns here attempt to accommodate minor date-time formatting
      // differences between JDK
      // versions. Ideally Gson would serialize in a way that is independent of the
      // JDK version.
      // Note: \h means "horizontal space", because some JDK versions use Narrow No
      // Break Space
  ...
  public void testGsonDateFormat() {
    ...
    try {
      ...
      // The expectation is that the date, after deserialization, when serialized
      // again should still
